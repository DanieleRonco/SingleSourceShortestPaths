...
(defun sssp (graph-id source-vertex)
  (progn (sssp-change-dist graph-id source-vertex 0)
	 (new-heap
	  graph-id
	  (length (graph-vertices graph-id)))
	 (let
	     ((array (graph-vertex-neighbors graph-id source-vertex)))
...
(defun sssp-aus (graph-id)
  (cond ((heap-not-empty graph-id)
	 (let ((vertex (second(heap-extract graph-id))))
	   (let ((array (graph-vertex-neighbors graph-id vertex)))
            (let ((dim (length array)))
...
(defun vertex-up-aus (graph-id vertex array point)
  (let ((vert (fourth (nth point array)))
	(dim (fifth(nth point array))))
    (let ((old-dist (sssp-dist graph-id vert))
	  (new-dist
	   (+
	    (sssp-dist graph-id vertex)
	    dim)))
      (cond ((equal old-dist nil)
	     (progn
...
(defun sssp-shortest-path-aus (graph-id source vertex elements)
  (let ((vertex-p (sssp-previous graph-id vertex)))
